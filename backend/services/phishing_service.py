"""
TrustCore Sentinel X — Phishing Detection Service

Uses a real TF-IDF + Naive Bayes classifier trained on labeled phishing/legit phrases.
No internet download required. Trains instantly on startup (<1s).
...
# ---------------------------------------------------------------------------
# Training Data — carefully curated phishing vs legitimate samples
# ---------------------------------------------------------------------------
_PHISHING_SAMPLES = [
...
# ---------------------------------------------------------------------------
# Additional heuristic signals (boosts confidence)
# ---------------------------------------------------------------------------
_PHISHING_PATTERNS = [
    ...
    r"\bipe\b",  # IP in email link pattern
]
...
def analyze_phishing(text: str) -> dict:
    ...
    # ML probability
    proba = _pipeline.predict_proba([text])[0][1]  # P(phishing)
    ...
    return {
        ...
        "signals": signals[:5],  # top 5 matched patterns
    }
